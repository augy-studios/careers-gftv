...
| `status-probe/` | The status probe behind `/status`: four public GETs a minute from the VPS, recorded in Supabase, and nothing to do with Telegram. Its own directory since 13 September 2026, with its own virtualenv, `.env`, lock and log, because living beside the bot got it mistaken for part of one and left unstarted for a fortnight. `setup.md` in it has the tmux window and the systemd unit. |
...
| [`status-probe/README.md`](status-probe/README.md) | What the probe records and refuses to, the four targets, and how to tell it is working. [`status-probe/setup.md`](status-probe/setup.md) is getting it running and keeping it running across a reboot. |
...
| `check-copy.js` | Every English string a reader can see, against the house style above. Reads nine sources: both dictionaries, the pages with their comments stripped out, the phase list, `llms.txt`, both documentation trees, every quoted string in the bot's Python, its About and Description text, and the translated guide pages — around 6,500 strings today — and exits non-zero on a banned phrase, a sentence over 25 words, or a Mainland word where 3a wants a Singapore one. It names the key and the sentence around it. `--list` prints what it reads, what is banned, and the cap. |
...
| `gen-screenshots.js` | The install screenshots both manifests declare: the portal's from `/search` and the docs site's from its home page, on the deployments, in the Hello theme, light mode. Rendered above 1x so the text in an install dialog is sharp. `node gen-screenshots.js main` or `docs` does one site. Rerun after the board changes. |
