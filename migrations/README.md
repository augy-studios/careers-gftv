...
| `041_mandarin_portal_name.sql` | Phase 14 part 4a. The 华文 half of `gftvjobs_settings.portal_title`, changed from 国际兽视 Careers to 国际兽视入队平台. **The half of that rename no dictionary could reach**: the home page's title is a row an admin can edit, stored per locale since `018`, and without this file the rename was complete everywhere except the largest words on the home page. It fires only on the exact string `018` wrote, so an admin who has since edited their own title keeps it. This row was missing from this list until part 9 added it. |
| `042_docs_translations.sql` | Phase 14 part 9. `gftvjobs_docs_translations`, `gftvjobs_docs_pages`, and the view `gftvjobs_docs_public` over both. The first holds the 华文 of every documentation page, gated ones included, and carries no `access` key per 16e. The second is the English of the **public** pages only, mirrored one direction by `docs-site/scripts/build.js` for readers outside Vercel, with a check constraint refusing a `/staff` path. **The view is what makes the pair safe**: it inner joins the two, so a gated translation joins to nothing and cannot appear, which is how the Telegram bot's `/docs` reads guide pages with no tier logic of its own. Both tables are written only by the build, from files in the repository; editing a row by hand is undone by the next deploy. |
| `043_notify_application_confirmed.sql` | Phase 14's webhook confirmation notice, 11 September 2026. Adds `notify_application_confirmed` to `gftvjobs_telegram_links`, the toggle for the fourth notification kind: the form webhook, or an admin linking a submission by hand, recorded an application as submitted on the applicant's behalf. Defaults on, like the other three. **Apply it before pulling the bot**: the bot names every notify column in the select it reads a link with, and a bot carrying a column the table lacks cannot read a link at all. Applied by hand the day it was written. |
| `044_malay_tamil_locales.sql` | Phase 15 part 1, 12 September 2026. Two rows in `gftvjobs_locales`, `ms` and `ta`, both active and neither with a text search configuration, so both search by trigram as `016` describes. No schema change: `014` said adding a language would be an insert, and this is it, numbered so the database and the checkout agree about what exists. **Active is not published.** The public half is the `locale_ms` and `locale_ta` feature keys on `/admin/maintenance`, and nothing changes for a reader the day this is applied. Applied by hand on 12 September 2026, the day it was written. |
