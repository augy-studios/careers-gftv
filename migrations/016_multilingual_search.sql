-- 016_multilingual_search.sql
--
-- Replaces: gftvjobs_search_jobs, gftvjobs_suggest.
-- Spec:     section 3a (Languages), section 4 (job listing and search).
-- Run after: 014.
--
-- Both functions resolve a language and return that language's wording in the
-- ordinary column names, so nothing downstream branches on locale. The client
-- asks for a locale and renders what comes back.
--
-- The locale travels inside the filters object rather than as a new parameter.
-- Adding a fifth parameter would create an overload rather than replace the
-- function, and every four argument call would then be ambiguous.
--
-- Both functions are dropped before being created, for two different reasons.
-- gftvjobs_search_jobs keeps its signature but returns two more columns, and a
-- returns table clause is part of the function's return type, which create or
-- replace may not change. gftvjobs_suggest gains a parameter, which would make
-- a new overload rather than a replacement.
--
-- How a language is matched:
--
--   The default language  lives on gftvjobs_jobs and keeps the weighted
--                         tsvector from 009, ranked with ts_rank_cd, with
--                         ts_headline snippets and a trigram fallback for
--                         typos.
--
--   Any other language    lives in gftvjobs_job_translations and is matched by
--                         substring against that row's generated search_text,
--                         accelerated by its GIN trigram index, with a
--                         similarity fallback. No ranking beyond title
--                         closeness, and no snippet.
--
-- The asymmetry is not laziness, it is Han script. Postgres cannot segment it,
-- so a tsvector would hold one token per run of characters and a search for
-- part of a word would never match. gftvjobs_locales.text_search_config records
-- which languages Postgres can tokenise; when a language that can be tokenised
-- is added, such as Malay, giving it a real indexed tsvector is a new numbered
-- file and a column on the translations table, not a rewrite of this.
--
-- Only a translation with is_ready set is used. An unready one is invisible to
-- search and the posting is found, and shown, in the default language instead.

begin;

-- Dropped rather than replaced. This version returns two more columns than the
-- one in 010, locale and has_translation, and create or replace cannot change a
-- function's return type: the row shape defined by a returns table clause is
-- part of that type. Widening it is a drop and a create.
--
-- Safe to drop: nothing calls it yet. The search endpoints arrive in phase 3,
-- and this is the definition they will find.
...
begin
  ...
  select
    ...
    c.location, c.is_remote, c.commitment_type, c.status,
...
comment on function gftvjobs_search_jobs(text, jsonb, int, int) is
  'Multilingual search. Pass the language as filters->>''locale''. Every returned field is already in that language, falling back to the posting. has_translation says whether a ready translation was used. headline contains <mark> tags, sanitise before rendering.';
...
drop function if exists gftvjobs_suggest(text, text);

create function gftvjobs_suggest(q text, p_locale text default 'en')
...
begin
  ...
  -- One Han character is a real query. One Latin letter is not. Languages
  -- Postgres can tokenise are word separated and behave like English here.
  v_min := case
  ...
  (
    ...
    where j.status = 'published'
      and (coalesce(nullif(tr.title, ''), j.title) ilike '%' || v_q || '%'
           or coalesce(nullif(tr.title, ''), j.title) % v_q)
    order by
      case when coalesce(nullif(tr.title, ''), j.title) ilike v_q || '%' then 0 else 1 end,
      similarity(coalesce(nullif(tr.title, ''), j.title), v_q) desc,
      j.title asc
    limit 5
  )
  union all
  (
    ...
    where t.usage_count > 0
      and (coalesce(nullif(tt.name, ''), t.name) ilike '%' || v_q || '%'
           or coalesce(nullif(tt.name, ''), t.name) % v_q)
    order by
  ...
  (
    ...
    where d.is_active
      and (coalesce(nullif(dt.name, ''), d.name) ilike '%' || v_q || '%'
           or coalesce(nullif(dt.name, ''), d.name) % v_q)
...
comment on function gftvjobs_suggest(text, text) is
  'Grouped autocomplete in any active language. The minimum query length is one character for a language Postgres cannot tokenise and two otherwise.';

insert into gftvjobs_migrations (filename)
values ('016_multilingual_search.sql')
...
-- Rollback
--
-- Restores the monolingual versions from 010. Paste the two function bodies
-- from that file, then:
--
-- begin;
-- drop function if exists gftvjobs_suggest(text, text);
-- -- then recreate gftvjobs_suggest(text) and gftvjobs_search_jobs(...) from 010
-- delete from gftvjobs_migrations where filename = '016_multilingual_search.sql';
-- commit;
