...
//
// Written by gen-docs-lib.js from main-site/assets/js/i18n.js.
...
async function loadDictionary(locale) {
  ...
  try {
    ...
    console.warn(`[careers-gftv-docs] could not load the ${locale} dictionary:`, cause);
...
export function t(key, vars) {
  ...
  if (typeof value !== 'string') {
    ...
    if (dictionaries.has(DEFAULT_LOCALE) && !warnedKeys.has(key)) {
      warnedKeys.add(key);
      console.warn(`[careers-gftv-docs] no dictionary entry for "${key}"`);
    }
    return key;
  }
