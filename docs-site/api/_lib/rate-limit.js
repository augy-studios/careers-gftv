...
//
// Written by gen-docs-lib.js from main-site/api/_lib/rate-limit.js.
...
export async function checkLock(bucket, subject) {
  ...
  if (error) {
    ...
    console.error('[careers-gftv-docs] checkLock:', error);
...
export async function recordFailure(bucket, subject, config) {
  ...
  if (readError) {
    console.error('[careers-gftv-docs] recordFailure read:', readError);
  ...
  if (writeError) console.error('[careers-gftv-docs] recordFailure write:', writeError);
...
export async function clearFailures(bucket, subject) {
  ...
  if (error) console.error('[careers-gftv-docs] clearFailures:', error);
}
