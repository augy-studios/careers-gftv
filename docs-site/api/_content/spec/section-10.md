...
access: developer
order: 13
summary: Amended twice since it was written, both deliberately, and the second amendment is a real reversal rather than an exception.
---
...
1. **Applications and resumes**: handled entirely in Google Forms. The portal stores no files and builds no application form. No Supabase Storage, no uploads.
   ...
   **Amended twice since it was written, both deliberately, and the second amendment is a real reversal rather than an exception.** Recorded here so nobody has to reconstruct it from the code.
   ...
   - **Question sets on tasks**, per 7g, decided 21 August 2026. Four question types, a cap of twenty, answered once. A set may be sent to chosen applicants **or attached to a posting so that everyone who applies to it is asked**. That second half is a small form built and served by the portal, which is the thing the original sentence ruled out, so the sentence no longer reads literally.
   ...
   - **The cap is the boundary made enforceable.** Twenty questions is enough for a real follow up and far too few for an application, and it is what stops this growing into one a question at a time.

   If a set ever needs to be longer than the cap, needs a file, or needs to be answered before applying, the answer is still the Google Form and this decision has not moved that far.
2. **Admin access and roles**: a gftvhello account reaches the admin dashboard if `is_admin` is true **or** `is_editor` is true, and `is_approved` is true. Apply this same check on every admin API route, and on the docs site. `gftvjobs_admin_access` overrides the flag check either way, per migration `012`, and `is_approved` is required regardless and cannot be waived.
   ...
   - **`is_admin`** is an **admin**. Admins are the developers of this project and are job posters as well, so they see everything: the applicant guides, the job poster guide, the admin guide, and the developer guide.
   ...
   - An account allowed in by a `gftvjobs_admin_access` override but holding neither flag is treated as a **job poster**, the lesser of the two. An override grants entry, not seniority, and the safe reading of an ambiguous grant is the smaller one.
   ...
   **An admin has full control over everything this portal configures.** Every posting whoever wrote it, every applicant, every application, every task, every department and tag, the portal settings in 8.10, the maintenance switches in 8.12, and the translation queue in 8.11. A job poster's work is not private from an admin and an admin may override any of it. **The `is_ready` flag is not on this list**, amended 4 September 2026 in phase 14 part 6: no helper may set it and any staff account may, per 7i as amended. Deviation 131.

   **What only an admin may do**, which is the same list stated from the other side, and is what the dashboard hides rather than disables for a job poster:
...
9. **The docs site has a staff login**, per section 16, and what a reader sees depends on their role per item 2. It signs staff in itself, with its own functions, its own cookie, and its own session table, rather than borrowing a session across two origins or widening a cookie to the parent domain.
...
12. **Staff get account recovery codes**, per 5g, and they set a gftvhello password. This is a deliberate exception to section 2's rule against writing to `gftvhello_*` tables and to 8.8's rule that password reset belongs to gftv.asia. It was chosen with that conflict stated. It covers `password_hash` and nothing else, it changes the account's gftv.asia password too, and the flow says so on screen.
