...
**Two questions are built in and are never part of a poster's set**: "did you apply for this role" and the posting rating, both from 7c. They belong to the apply prompt, they are derived from `gftvjobs_analytics` rather than stored as questions, and no composer can edit, reorder, or remove them. A poster's questions never appear inside the handoff modal either, for the reason 7c gives: that modal is a light tap on the shoulder, not an ambush, and hanging a required form off it would make dismissing it cost something.
...
- **The posting, from now on.** The set is stored on the posting, and every applicant who applies from that point gets a task raised automatically at the moment they are handed over. Existing applicants are untouched unless the poster also sends to them. **This is a form asked of everybody who applies, and section 10 item 1 has been amended to permit it. Read that item before extending this.**

A poster sending to more than one applicant sees exactly who will receive it before anything is written, per the same rule 8.5 applies to bulk invites. Each recipient gets an independent task with its own frozen copy of the questions, so resolving one, or a set going out wrongly, is per applicant and never one shared row.

The set itself:

- **Four question types, and no more without a decision**: **short answer** (one line), **long answer** (a paragraph), **choice** (pick exactly one of a list), and **checkbox** (pick any number of a list, including none). A single yes or no confirmation is a checkbox question with one option.
...
- **Questions are written in every shipped language, and displayed in the reader's own**, per 3a. The composer offers a tab per language exactly as the job editor does, because unlike a message to one named person, a set can now reach everybody who applies and their languages differ. **A language left blank falls back to one that was written**, which is the same rule every other string on this site follows; it never renders empty.
...
- **A free text box is always offered alongside the questions and cannot be turned off.** Somebody asked three specific things often needs to say a fourth, and no way to say "none of these quite fit" collects worse answers than one that has one.
- **Answers are validated on the server against the question set stored on that task**, never against what the browser sends back. Every required question must be answered, an answer to a choice or checkbox question must be one of that question's own option values, and an answer naming a question the task does not carry is rejected rather than stored.
...
- The same control appears wherever else translated content is shown at length, and on the interface itself: a reader who spots a bad label in the navigation should be able to report it without working out that it is an interface string rather than part of a posting. The report form asks what is wrong, not what kind of thing is wrong.
- Opening it shows: which language version has the problem, defaulting to the one currently being read; which part, defaulting to the whole posting; a box for what is wrong; and an optional box for a better wording.
...
- Reports are stored in `gftvjobs_translation_reports`. They are not tasks: they are outbound from the applicant rather than something the portal needs from them, so they do not belong on `/account/tasks` and must not add to its badge count. An admin who needs to ask a follow up question raises an ordinary `info_request` task, which is what that table is for.
...
- A translation helper is an ordinary `gftvjobs_users` account that an admin has granted the role for one language. They are language speakers, not staff, and deliberately do not go through `gftvhello` or the admin access overlay: the person best placed to fix the Chinese has no reason to be a GFTV staff member.
...
The helper area is not the only way in. A helper reading any page of the site can select the wording that reads wrongly and suggest a replacement for that exact span, in place, without leaving the page or knowing what a dictionary key is.
