...
access: developer
order: 18
summary: Build a Telegram bot in a new telegram-bot directory in this same repo, based on the scripts in main-site so the two agree on the schema and the flows.
---
...
Added 26 August 2026, built in phase 12, and it has nothing to do with Telegram. It lives here because of where it runs rather than what it does: the status page in 0c needs a prober outside Vercel, and **this VPS is the only thing in the whole architecture that is**. Standing up a second machine for a loop that makes four requests a minute would be worse than putting it beside a process that is already running, already has the Supabase credentials, and is already in this repository.

- A loop, separate from the bot's own event loop and able to fail without taking the bot down. If Telethon is wedged the probe should still be recording, since "the bot is broken" and "the portal is down" are exactly the two things a status page has to tell apart.
- Every sixty seconds, request `/api/public/feature-status`, `/search`, one seeded posting page, and `/api/public/jobs.json`. Report all four to `gftvjobs_status_record()` in one call, with the status code, the duration and whether each succeeded; the function keeps the day's counters and the outage rows in section 6. Public and read only, all four, so this needs no portal credential and can never change anything. **Which posting is probed is read from the public feed** rather than configured, so it follows the board when the seed is cleared.
...
- **A failure to reach Supabase writes nothing.** No local buffer and no backfill on reconnect: a gap is drawn on the page as unknown, which is true, and a backfilled row timestamped an hour late is not.
