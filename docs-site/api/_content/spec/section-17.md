...
- The `docs-site` directory per section 16, with its own README, its own `.env.example`, its own `api/` and staff login per 5h, the role gate per 16a, the four guides per 16h, the Playwright capture script, and the screenshot manifest.
