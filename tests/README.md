...
labelled by page and language instead: their count moves the day somebody adds a
page, and numbering them would renumber the file every time the site grows.
...
| `account` | **5f's settings suite and the two pages with no article.** Ten rules read out of the source, because they are about the whole of both projects and not about one file: which files write `gftvhello_users` and which columns they write, that the danger zone route and the page agree on all six actions and that none of them deletes the account, that every class the shared module writes is defined on the docs site, that the two code sets are four distinct tables, and that every route writing `gftvhello_users` sits behind `HELLO_WRITES_ENABLED` with 5g held before a recovery code is ever verified. **Its output names whether the hold is on**, so a tree with it lifted and the walk not done is visible. Then a browser over `/account`, `/login` and `/forgot-password`, with fixtures chosen for the awkward cases: a code count that could not be read, a passkey registered on the other site, a session on each. |
| `live` | **The same questions, asked of the deployment**, and as of part 6 its three new addresses too: that `/login`, `/account` and `/forgot-password` are served by the shell, that every module they import reached `dist/`, that the hold answers 503 with `reason: "held"` on both sites, and that a stranger is refused before the hold is ever mentioned., and the only section here that touches the network. It needs no credential, because everything it asks it asks as a stranger: the built pages are files, the content tree is not served as markdown, the route answers by parameter and not by path, a gated page is 404 and never 401, the public index holds nothing from the staff half, and the payload carries a date — which is the proof that `includeFiles` reached `api/_generated/` and that the build ran before the functions were packaged. `DOCS_BASE=` points it somewhere else. |
| `responsive` | **Deviation 118's half of the pass, arriving one phase later.** Six widths from 320px, every page in both trees plus `/login`, `/account` and `/forgot-password`, as a signed out reader and as one who can see everything, in English and 华文. No sideways scroll, no squeezed table cell outside a scroller, no short control label wrapping, and 16d's own sentence: search and the account control keep their place in the header at every width. Then the three column arrangement, measured on the run's own fixture page because it is the only address on this site with a second heading — `.docs-toc:empty` collapses the contents column, so a placeholder would have reported it missing everywhere. |
| `a11y` | The same nine rules phase 12 part 2 wrote, over the same pages at 375 and 1024 in both languages: an accessible name on everything reachable by Tab, nothing focusable inside `aria-hidden`, every ARIA reference resolving, no id used twice, one `h1` with no level skipped, an alt on every image, no positive tabindex, and the skip link first and landing. **It proves it can fail before it is trusted**, with a nameless control and a link inside `aria-hidden` injected into a real page. Then the sidebar panel a keyboard has to be able to drive, which is where part 7's one defect was: below 640px the panel closes by moving off the edge, so it was displayed, in the tab order and in the accessibility tree the whole time. |
| `contrast` | **Phase 12 part 3's lesson asked of this site's own components**, in all four theme combinations: a token clearing AA says nothing about a component that puts one token on another. Around forty pairs — the chrome, everything `markdown.js` draws, and the search panel — each composited against what is really behind it and held to the threshold its own size and weight earn it. Three 1px separators are advisory and printed with their numbers, because 1.4.11 is about identifying components and states and a rule that divides one region from the next identifies nothing; **what carries those states is checked instead**, so no tab is selected in hue alone and no callout is a tint with no word on it. |
| `seam` | The second project as a project: its four variables in `env.js`, in `.env.example` with a comment above each, and in the root README's table, in both directions; that `SITE_URL` is documented as the portal and not this site, per 5e; the build command, the output directory, the `includeFiles` glob and the catch-all rewrite in `vercel.json` and not in a settings form; that the READMEs describe the site that is actually here; and that the phase does not read `shipped` while the three gftv.asia writes are still held. **Its output names whether the hold is on**, the same as `account`'s. |
