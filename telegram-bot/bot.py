...
Telethon's event loop and, from part 4, the outbox drain live in this one
process and share one client and one session file. The drain has to send through
Telethon anyway, and a second process would mean a second session for the same
bot token; safety against a double send comes from the conditional claim in the
database, not from there happening to be one process. Phase 12's status probe is
the exception and stays separate, because section 15 requires it to keep
recording while Telethon is wedged: telling "the bot is broken" apart from "the
portal is down" is the entire job of a status page.
...
async def run(config: Config) -> int:
    ...
    async with httpx.AsyncClient(headers={"User-Agent": USER_AGENT}) as http:
        ...
        await asyncio.gather(disconnected, waiting, return_exceptions=True)
