...
A copy of the bot's lock.py, made when the probe moved into its own directory
on 13 September 2026, so the probe imports nothing from the bot. The lock file
here is `probe.lock`, and two probes would write two rows a minute where one
was measured. The reasoning below is the bot's and holds here unchanged.
