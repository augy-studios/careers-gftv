...
export async function recordAudit(entry) {
  try {
    const { error } = await supabase.from(T.auditLog).insert({
      ...
      // The site last, so it is written even when a caller passes a metadata
      // object of its own, and so a caller cannot claim to be the other one.
      metadata: { ...(entry.metadata ?? {}), site: SITE },
    });
