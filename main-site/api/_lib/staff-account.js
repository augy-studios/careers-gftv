...
export async function staffProfile(userId) {
  ...
  if (error) {
    console.error('[careers-gftv] staffProfile:', error);
...
export async function setStaffPassword(userId, plain) {
  ...
  if (error) {
    console.error('[careers-gftv] setStaffPassword:', error);
    return false;
  }
...
export async function setStaffTotpSecret(userId, secret) {
  ...
  if (error) {
    console.error('[careers-gftv] setStaffTotpSecret:', error);
    return false;
  }
...
export async function listStaffSessions(userId, currentSessionId = null) {
  ...
  for (const { site, table } of SESSION_TABLES) {
    ...
    if (error) {
      ...
      console.error(`[careers-gftv] listStaffSessions ${site}:`, error);
      failed = true;
      continue;
    }
...
export async function revokeStaffSessions(userId, options = {}) {
  ...
  for (const { site, table } of SESSION_TABLES) {
    ...
    if (error) {
      console.error(`[careers-gftv] revokeStaffSessions ${site}:`, error);
      ok = false;
    }
  }
...
export async function invalidateCodeSet(set, userId) {
  ...
  if (error) {
    console.error('[careers-gftv] invalidateCodeSet:', error);
