...
| `api/account/telegram` | section 15's applicant half: the linking token and its QR, the link state the page polls, the unlink, the second factor switch, the test message, and a code push for 7g's danger zone. **`start` returns a QR matrix rather than an image**, because an image URL would put a linking token, which is a credential, into something every proxy logs | 11 |
| | **A login code is asked for here and generated by the bot.** The site writes a row with `token_hash` set to `pending:` and randomness, and the bot claims it, generates six digits, sends them, and writes back the bcrypt hash. The site cannot generate it: it has no way to deliver it, and a code it wrote would have to cross the table in the clear to reach something that has. | |
| `api/auth/applicant/magic` | the one tap sign in from section 15. A full login rather than a second factor, bound to the browser that asked by a nonce cookie, five minutes, single use. **A request carrying no nonce cookie does not spend the token**, because unfurlers and link checkers fetch URLs without cookies and burning somebody's sign in before their thumb reached it would look exactly like a broken bot. GET only, and the second deliberate exception to the HEAD rule | 11 |
...
Passkeys are the second factor in **both** realms, added in phase 2. They were
what gave the applicant realm a second factor at all until phase 11 part 3
finished the Telegram half, and the two now sit side by side: either satisfies
the step, an account with both is offered both, and turning one on never takes
the other away.
